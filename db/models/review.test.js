...
describe('Review', () => {
	...
	describe('Working Associations', () => {
		it('Reviews Belong to Product', () => {
			...
			return Promise.all([creatingProduct, creatingReview]).then(([createdProduct, createdReview]) => {
		...
		it("Reviews Belongs to User", () => {
			...
			return Promise.all([creatingUser, creatingReview]).then(([createdUser, createdReview]) => {
	...
	describe('Working Validation', () => {
		it("Fails if content < 10 characters", () => {
			return Review.create({title: 'Review Test', content: 'short', numStars: '3'}).then(review => {
		...
		it("Passes if content >= 10 characters", () => {
			return Review.create({title: 'Review Test', content: 'this is a lot of content', numStars: '3'}).then(result => {
